...
class DuelGame(bs.TeamGameActivity):
    ...
    def getDescription(cls,sessionType):
        return 'Defeat your enemies in 1v1 battles.'
    ...
    def getInstanceDescription(self):
        return ('Defeat ${ARG1} enemies in 1v1 battles.', self.settings['Kills to Win Per Player'] if isinstance(self.getSession(), bs.FreeForAllSession) else self.settings['Kills to Win Per Team'])
            ...
            if killerTeam is player.getTeam():
                if isinstance(self.getSession(),bs.FreeForAllSession):
                    if self.settings['Reduce score on suicide']:
                        if self.settings['Allow negative scores']:
