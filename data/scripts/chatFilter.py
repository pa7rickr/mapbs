...
def abusecheck(msg):
    sp=msg.split()
    for f in sp:
        if (f in list1.split()) or (f in list2.split()) or (f in list3.split()) or (f in list4.split(', ')) :
            return True
            break
        else:
            pass
...
def addwarn(cid):
    ...
    if cid in data:
        ...
        data[str(cid)] = data.get(str(cid), 0) + 1
    with open("abusers.json",mode='w') as f:
        json.dump(data, f, indent=4)
    warns = data.get(str(cid))
    return warns
...
def chat(msg,clientID):
    if bsInternal._getForegroundHostActivity() is not None:
        if abusecheck(msg):
        	if True:
        	    d.abusechatsave(msg)
        	    if d.checkOwner(clientID) == True:
                        pass
        	    else:
                        pbid = d.checkOwner(clientID)
                        warns = addwarn(pbid)
                        if warns == kickat:
                            bs.screenMessage("Kicked player for abusing!",color=(1,0,0), clients=[clientID], transient=True)
                            setwarn(pbid)
                            bsInternal._disconnectClient(clientID)
                            
                        else:
                            bs.screenMessage('Don\'t abuse here!',color=(1,0,0), clients=[clientID], transient=True)
                            bs.screenMessage('Warns left:{}/{}'.format(warns,kickat),color=(1,0,0), clients=[clientID], transient=True)
        else:
            pass        
#ADD YOUR CUSTOM BAD WORD LIST HERE 
list1='ajg anjing anjg cuk cok kntl kontol mf 69 fuck mc bc lawde chutiya mother f**** laude chutiye maderchod maderchhod maderchood madarchod madarchhod chut bhen bsdk kaminey gaand g*** tatti pennis fucker madarchod machod chood machod f.u.c.k fck fkuc fuk sex sexy nigga'
