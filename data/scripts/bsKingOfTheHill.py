...
class KingOfTheHillGame(bs.TeamGameActivity):
        ...
        self._flagRegionMaterial.addActions(
            ...
            actions=(("modifyPartCollision", "collide", True),
                     ...
                     ("call", "atConnect", bs.Call(
                         self._handlePlayerFlagRegionCollide, 1)),
                     ("call", "atDisconnect", bs.Call(
                         self._handlePlayerFlagRegionCollide, 0))))
    ...
    def onBegin(self):
        ...
        bs.newNode('region',
                   attrs={'position': self._flagPos,
                          'scale': (1.8, 1.8, 1.8),
                          'type': 'sphere',
                          'materials': [self._flagRegionMaterial,
    ...
    def endGame(self):
        ...
        for team in self.teams:
            results.setTeamScore(
                team, self.settings['Hold Time'] - team.gameData
                ['timeRemaining'])
        self.end(results=results, announceDelay=0)

    def _updateFlagState(self):
        holdingTeams = set(player.getTeam()
                           for player in self.players
                           if player.gameData['atFlag'])
    ...
    def _handlePlayerFlagRegionCollide(self, colliding):
        ...
        # also don't count it if we're dead (flying heads shouldnt be able to
        # win the game :-)
        if colliding and player.isAlive():
            player.gameData['atFlag'] += 1
        else:
            player.gameData['atFlag'] = max(0, player.gameData['atFlag'] - 1)
