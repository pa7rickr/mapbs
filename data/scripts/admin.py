...
class PermissionEffect(object):
    def __init__(self, position=(1,0, 0), owner=None, prefix='ADMIN', prefixColor=(1, 1, 1),
        ...
        bs.animate(self._Text, 'scale', {0: 0.0, 1200: 0.0080})  # smooth prefix spawn
...
class Enhancement(bs.Actor):
	...
	def getTag(*args):
		...
		if cl_str in gph.effectCustomers:
			...
			if effect == 'ice':
				...
				self.metalTimer = bs.Timer(5000, bs.WeakCall(self.emitMetal), repeat=True)
    ...
    def emitMetal(self):
        ...
        if spaz is None or not spaz.isAlive() or not spaz.node.exists():
            self.handleMessage(bs.DieMessage())
            return
        bs.emitBGDynamics(position=spaz.node.position, velocity=spaz.node.velocity, count=random.randint(2,8), scale=0.4, spread=0.7,
                          chunkType="metal")
    def handleMessage(self, m):
